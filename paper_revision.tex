...
\abstract{
Understanding future changes in extreme temperature events is critically important because of their potential societal cost, but inherently challenging in a transient climate. A single model simulation is generally insufficient to empirically characterize the statistical properties of underlying processes. Ensembles of repeated simulations (with different initial conditions providing independence) not only alleviate this data insufficiency but also allow new approaches for characterizing changes in extremes. We present here a new method of using ensembles that allows characterizing changes in temperature distributions using a continuous representation of seasonality rather than breaking the dataset into seasonal blocks. That is, we assume that temperature distributions evolve smoothly both day-to-day over an annual cycle and year-to-year over longer secular trends. To demonstrate our method's utility, we analyze an ensemble of 50 simulations of the Community Earth System Model (CESM) under a scenario of increasing radiative forcing to 2100, focusing on North America. The results both confirm aspects of climate system behavior known from previous studies and also elucidate new features.Confirming results include that daily temperature bulk variability generally decreases  in wintertime in the continental mid- and high-latitudes ($> 40^{\circ}$). New results include that in these same wintertime distributions, the low tails ``stick'', i.e.\ experience a lesser reduction in variability, producing a more negative skew. Although the examples above concern temperature only, the technique is sufficiently general that it can be used to generate precise estimates of distribution changes in a broad range of climate variables by exploiting the power of ensembles.}
...
% CC BEGIN
 As an example of a typical model fit, we show in Figure \ref{Estimate} the seasonal cycle in CESM daily temperatures for three locations, along with estimates of low, median and high quantiles. We show here data from 1850 to demonstrate the seasonal fit rather than that of the long-term trend. All locations show strong seasonal differences in variance that are well-represented by our smooth estimates. Relevant features that are captured include an asymmetrical seasonal cycle in all locations; a clear left skewness in wintertime in all three locations (although most pronounced in the higher-latitude \textbf{a} and \textbf{b}); and a distinct springtime shoulder in the higher-latitude locations. %All three sites show that the transition from cold to warm temperatures in spring looks different from the warm to cold transition in fall, suggesting 
...
% \EPSFIG[scale=0.45]{skewness.pdf}{Change in temperature skewness between the first and last 10 years of the model simulations. For winter, we use the first 10 days of the year and for summer we use July 5-15 to calculate the sample skewness. Gray crosses represent points where the skewness change is not significant at the $\alpha=0.95$ level, obtained by resampling the set of 50 simulations (with replacement) and recalculating the sample skewness. After 100 repetitions of the resampling, we calculate the $95^{th}$-level confidence interval to determine if the skewness change is significantly different from zero. Each color bar represents the maps above and below it.}{skewness}
...
estimated using our quantile model, and for fractional changes see Supplementary Online Material Figure \ref{dq_vs_day_norm}.  %Again, each locations' temperature distributions are shown in Figure \ref{location_distributions} for summer and winter. 
  %  shows variability both in the bulk and in the tails for each day of the year 
